...
public class PaletteBuilder {
	...
	public function showBlocksForCategory(selectedCategory:int, scrollToOrigin:Boolean, shiftKey:Boolean = false):void {
		...
		if (selectedCategory == Specs.obsoleteCategory) return showObsoleteCategory();
	...
	private function addBlocksForCategory(category:int, catColor:int):void {
		...
		for each (var spec:Array in Specs.commands) {
			if ((spec.length > 3) && (spec[2] == category)) {
				var blockColor:int = (app.interp.isImplemented(spec[3]) || category > 200) ? catColor : 0x505050;
	...
	private function showObsoleteCategory():void {
		var workingCategories:Array = [Specs.soundCategory];
		var brokenCategories:Array = [Specs.motionCategory, Specs.looksCategory, Specs.controlCategory, Specs.sensingCategory];
		addItem(makeLabel(Translator.map('Working')));
		for each (var cat:int in workingCategories) addObsoleteBlocksFor(cat, true);
		addItem(makeLabel(Translator.map('Broken')));
		for each (cat in brokenCategories) addObsoleteBlocksFor(cat, false);
		addItem(makeLabel(Translator.map('Glitch')));
		var undefinedColor:int = Specs.blockColor(0);
		addItem(new Block('--', null, undefinedColor, null));
	}
	
	private function addObsoleteBlocksFor(category:int, working:Boolean):void {
		addBlocksForCategory(category + (working ? 200 : 300), Specs.blockColor(category));
		nextY += 10;
	}
