...
public class BlockMenus implements DragClient {
	...
	private function keyMenu(evt:MouseEvent):void {
		function setKeyArg(k:*):void {
			switch (k) {
			case 'backspace': blockArg.setArgValue('\u0008', Translator.map('backspace')); break;
			case 'enter': blockArg.setArgValue('\r', Translator.map('enter')); break;
			case 'tab': blockArg.setArgValue('\t', Translator.map('tab')); break;
			case 'escape': blockArg.setArgValue('\u001b', Translator.map('escape')); break;
			case 'delete': blockArg.setArgValue('\u007f', Translator.map('delete')); break;
			case 'special': blockArg.setArgValue('', Translator.map('special')); break;
			default: blockArg.setArgValue(k);
			}
		}
		var ch:int;
		var namedKeys:Array = ['up arrow', 'down arrow', 'right arrow', 'left arrow', 'space'];
		var hackedNamedKeys:Array = ['backspace', 'enter', 'tab', 'escape', 'delete', 'special'];
		var m:Menu = new Menu(setKeyArg, 'key');
		for each (var s:String in namedKeys) m.addItem(s);
		if (block.op == 'keyPressed:') for each (s in hackedNamedKeys) m.addItem(s);
		for (ch = 97; ch < 123; ch++) m.addItem(String.fromCharCode(ch)); // a-z
		for (ch = 48; ch < 58; ch++) m.addItem(String.fromCharCode(ch)); // 0-9
		if (block.op == 'keyPressed:') {
			for (ch = 33; ch < 48; ch++) m.addItem(String.fromCharCode(ch)); // !-/
			for (ch = 58; ch < 65; ch++) m.addItem(String.fromCharCode(ch)); // :-@
			for (ch = 91; ch < 97; ch++) m.addItem(String.fromCharCode(ch)); // [-`
			for (ch = 123; ch < 127; ch++) m.addItem(String.fromCharCode(ch)); // [-`
		}
		showMenu(m);
	}
	...
	private function mathOpMenu(evt:MouseEvent):void {
		var ops:Array = ['abs', 'floor', 'ceiling', 'int', 'sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'ln', 'log', 'e ^', '10 ^'];
	...
	private function addGenericBlockItems(m:Menu):void {
		if (!block) return;
		var inPalette:Boolean = isInPalette(block);
		m.addLine();
		if (!inPalette) {
			if (!block.isProcDef()) {
		...
		m.addItem('view op',  block.viewOp);
		if (!inPalette) m.addItem('modify JSON', block.modifyJSON);
		m.addLine();
		if (!inPalette) m.addItem('add comment', block.addComment);
