...
public class Watcher extends Sprite implements DragClient {
	...
	public function doubleClick(evt:MouseEvent):void {
		...
		if (!targetIsVariable() && (newMode == 3)) newMode = 4;
	...
	public function menu(evt:MouseEvent):Menu {
		...
		m.addItem("fatty readout", 4);
		if (targetIsVariable()) m.addItem("slider", 3);
		if (targetIsVariable() && (mode == SLIDER_MODE)) {
			m.addLine();
