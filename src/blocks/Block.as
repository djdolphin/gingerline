...
//	private static const blockLabelFormat:TextFormat = new TextFormat('LucidaBoldEmbedded', 10, 0xFFFFFF, true);
	...
	private function argOrLabelFor(s:String, c:int):DisplayObject {
		...
		if (s.length >= 2 && s.charAt(0) == "%") { // argument spec
			...
			if (argSpec == "h") return new BlockArg("h", c, true);
	...
	public function modifyJSON():void {
		var b:Block = this;
		var d:DialogBox = new DialogBox(function():void {
			var newBlock:Block = BlockIO.arrayToBlock(util.JSON.parse(d.getField('json')), b.isReporter ? 'r' : ' ', Scratch.app.viewedObj().isStage),
				p:DisplayObject = b.parent;
			newBlock.nextBlock = nextBlock;
			if (nextBlock) newBlock.addChild(nextBlock);
			if (p is Block) {
				var pb:Block = Block(p);
				if (pb.nextBlock == b) {
					pb.removeChild(b);
					pb.nextBlock = newBlock;
					pb.addChild(newBlock);
					pb.fixStackLayout();
				} else if (pb.subStack1 == b) {
					pb.removeChild(b);
					pb.subStack1 = newBlock;
					pb.addChild(newBlock);
					pb.fixStackLayout();
				} else if (pb.subStack2 == b) {
					pb.removeChild(b);
					pb.subStack2 = newBlock;
					pb.addChild(newBlock);
					pb.fixStackLayout();
				} else {
					var i:int = pb.labelsAndArgs.indexOf(b);
					if (i < 0) return;
					pb.removeChild(b);
					pb.labelsAndArgs[i] = newBlock;
					pb.addChild(newBlock);
					pb.fixArgLayout();
				}
			} else {
				...
				newBlock.x = b.x;
				newBlock.y = b.y;
				newBlock.fixStackLayout();
			}
		});
		d.addTitle('Modify JSON');
		d.addField('json', 200, util.JSON.stringify(BlockIO.blockToArray(this), false), false);
		d.addAcceptCancelButtons('Set');
		d.showOnStage(stage);
	}
	
	public function viewOp():void {
		var d:DialogBox = new DialogBox(), b:Block = this;
		d.addTitle('View Op');
		d.addText(op)
		d.addButton('Copy', function():void {
			Clipboard.generalClipboard.clear(); 
			Clipboard.generalClipboard.setData(ClipboardFormats.TEXT_FORMAT, b.op, false); 
		}, false);
		d.addButton('Close', null);
		d.showOnStage(stage);
	}
