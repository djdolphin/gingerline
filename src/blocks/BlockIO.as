...
public class BlockIO {
	...
	public static function blockToArray(b:Block):* {
		...
		if (b.op == Specs.CALL) return [Specs.CALL, b.spec];			// procedure call - arguments follow spec
		if (b.op == 'true') return true;
		if (b.op == 'false') return false;
		if (b.op == 'null') return null;
		if (b.op == 'Infinity') return Number.POSITIVE_INFINITY;
		if (b.op == '-Infinity') return Number.NEGATIVE_INFINITY;
		if (b.op == 'NaN') return NaN;
		if (b.op == 'hexValue') return new HexInt(b.args[0].argValue);
	...
	public static function arrayToBlock(cmd:*, undefinedBlockType:String, forStage:Boolean = false):Block {
		// Make a block from an array of form: <op><arg>*

		if (!(cmd is Array)) return valueToBlock(cmd, undefinedBlockType);
		...
		if (b.op == 'keyPressed:') fixKeyRefs(b);
		b.fixArgLayout();
		return b;
	}
	
	private static function valueToBlock(cmd:*, undefinedBlockType:String):Block {
		var cmdString:String;
		if (cmd is Boolean) cmdString = cmd.toString();
		if (cmd == null) cmdString = 'null';
		if (cmd == Number.POSITIVE_INFINITY) cmdString = 'Infinity';
		if (cmd == Number.NEGATIVE_INFINITY) cmdString = '-Infinity';
		if (cmd is Number && isNaN(cmd)) cmdString = 'NaN';
		if (cmd is HexInt) cmdString = 'hexValue';
		var spec:Array = specForCmd([cmdString], undefinedBlockType);
		var b:Block = new Block(spec[0], spec[1], Specs.blockColor(spec[2]), spec[3]);
		if (cmdString == 'hexValue') b.setArg(0, cmd.toString());
		return b;
	}
	...
	private static function argsForCmd(cmd:Array, reverseArgs:Boolean):Array {
		...
		for (var i:int = 1; i < cmd.length; i++) {
			var a:* = cmd[i];
			if (a is String || (a is Number && isFinite(a))) result.push(a); //literal value
			else if (!(a is Array && a[0] is Array)) result.push(arrayToBlock(a, 'r')); // block (skip if substack)
		}
	...
	private static function fixKeyRefs(b:Block):void {
		var arg:BlockArg;
		if (b.args[0] is BlockArg) arg = b.args[0];
		if (arg) {
			switch (arg.argValue) {
			case '\u0008': arg.setArgValue('\u0008', Translator.map('backspace')); break;
			case '\r': arg.setArgValue('\r', Translator.map('enter')); break;
			case '\t': arg.setArgValue('\t', Translator.map('tab')); break;
			case '\u001b': arg.setArgValue('\u001b', Translator.map('escape')); break;
			case '\u007f': arg.setArgValue('\u007f', Translator.map('delete')); break;
			case '': arg.setArgValue('', Translator.map('special')); break;
			}
		}
	}
